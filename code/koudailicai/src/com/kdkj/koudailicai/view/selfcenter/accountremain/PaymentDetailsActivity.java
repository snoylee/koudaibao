...
public class PaymentDetailsActivity extends BaseActivity  implements HttpErrorInterface{
	...
	private void cessionRefresh(){
		...
		lvPaymentDetails.setOnRefreshListener(new OnRefreshListener<ListView>() {
			...
			public void onRefresh(PullToRefreshBase<ListView> refreshView) {
				...
				errorListener.setErrInterface(item.size()>0 ? null : PaymentDetailsActivity.this);
		...
		public void onResponse(JSONObject response) {
			...
			try {
				if (response.getInt("code") == 0) {
        			...
        			dialog = KdlcDialog.showLoginDialog(PaymentDetailsActivity.this);
        		} else {
					if(item.size() <= 0) {
						showErrReq();
					}
					dialog =KdlcDialog.showInformDialog(PaymentDetailsActivity.this,response.getString("message"));
				}
	...
	private void initTitle() {
		// TODO Auto-generated method stub
		title.setTitle(R.string.payment_details);
